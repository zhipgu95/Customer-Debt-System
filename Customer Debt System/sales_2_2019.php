...
<link href="css/sales_year.css" rel="stylesheet"/>
</head>
<body>
...
<div class = "include">
	...
	<div class = "table_info">
		...
		//	$start = $total - $perpage;
			...
			$sql = "SELECT Shop_no, YEAR(Date) AS year, MONTH(Date) AS month, SUM(Sales_amount) AS monthly_sales_amount, SUM(Costs_amount) AS monthly_costs_amount FROM sales_shop WHERE YEAR(Date) = 2019 AND Shop_no=2 GROUP BY month";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>商店号码</th><th>年份</th><th>月份</th><th>销售总额</th><th>费用总额</th></tr>";
				//output data of each row
				$total_sales_amount_2019 = 0;
				$total_costs_amount_2019 = 0;
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["Shop_no"]."</td><td>".$row["year"]."</td><td>".$row["month"]."</td><td>".$row["monthly_sales_amount"]."</td><td>".$row["monthly_costs_amount"]."</td></tr>";	
					$total_sales_amount_2019+=$row["monthly_sales_amount"];
					$total_costs_amount_2019+=$row["monthly_costs_amount"];
				}
			//	$total_debt+=$totaldebt_perpage;
				echo "</table>";
				echo "</div>";
				echo "<div id='debt_form'>";
					echo "<p>2018年总销售额: ".$total_sales_amount_2019."</p>";
					echo "<p>2018年总费用额: ".$total_costs_amount_2019."</p>";
				echo "</div>";
			}
			else {
				echo "0个查询结果";
		...
		<div class = "table3">
    	<form method = "Post" action = "sales_2.php" enctype="multipart/form-data">
