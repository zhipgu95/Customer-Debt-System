...
<div class = "include">
	...
	<div id="menu">
		...
		//	$start = $total - $perpage;
			...
			$sql = "select Demander, Provider, YEAR(Date) as Year, SUM(Amount) as Sum_Amount 
					from items_transfer 
					where Provider=8 and YEAR(Date)=2018 group by Demander";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>需求方</th><th>提供方</th><th>年份</th><th>金额数</th><th>具体查询</th></tr>";
				//output data of each row
				$total_amount_one = 0;
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["Demander"]."</td><td>".$row["Provider"]."</td><td>".$row["Year"]."</td><td>".$row["Sum_Amount"]."</td><td><a href='items_out_8_2018_detail.php?search=$row[Demander]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>8号店2018年出货金额总数: ".$total_amount_one."</p>";
				echo "</div>";
