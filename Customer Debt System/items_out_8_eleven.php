...
<div class = "include">
	...
	<div id="menu">
		...
		//	$start = $total - $perpage;
			...
			$sql = "select Demander, Provider, YEAR(Date) as Year, MONTH(Date) as Month, SUM(Amount) as Sum_Amount from items_transfer where Provider=8 and MONTH(Date)=11 and YEAR(Date)=2018 group by Demander";
			...
			if(!logged_in()){
				...
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["Demander"]."</td><td>".$row["Provider"]."</td><td>".$row["Year"]."</td><td>".$row["Month"]."</td><td>".$row["Sum_Amount"]."</td><td><a href='items_out_8_eleven_detail.php?search=$row[Demander]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>8号店11月份出货金额总数: ".$total_amount_one."</p>";
				echo "</div>";
