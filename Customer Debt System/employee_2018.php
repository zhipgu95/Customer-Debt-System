...
<div class = "include">
	...
	<div class = "table_info">
		...
		//	$start = $total - $perpage;
			...
			$sql = "SELECT distinct concat(er.LastName, ' ', er.FirstName) AS name, YEAR(er.Date) AS year, SUM(er.Amount) as amount
					FROM employee_record AS er 
					WHERE YEAR(er.Date)=2018 GROUP BY name ORDER BY amount DESC";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>姓名</th><th>年份</th><th>金额数</th><th>具体查询</th></tr>";
				//output data of each row
				$total_amount_one = 0;
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["name"]."</td><td>".$row["year"]."</td><td>".$row["amount"]."</td><td><a href='employee_2018_detail.php?search=$row[name]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>2018年员工借款金额总数: ".$total_amount_one."</p>";
				echo "</div>";
