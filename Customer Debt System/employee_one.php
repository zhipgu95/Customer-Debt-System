	...
	<div class = "table_info">
		...
		//	$start = $total - $perpage;
			...
			$sql = "SELECT distinct concat(er.LastName, ' ', er.FirstName) AS name, YEAR(er.Date) AS year, MONTH(er.Date) AS month, SUM(er.Amount) as amount
					...
					WHERE MONTH(er.Date)=1 AND YEAR(er.Date)=2018 GROUP BY name ORDER BY amount DESC";
			...
			if(!logged_in()){
				...
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["name"]."</td><td>".$row["year"]."</td><td>".$row["month"]."</td><td>".$row["amount"]."</td><td><a href='employee_one_detail.php?search=$row[name]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>2018年1月员工借款金额总数: ".$total_amount_one."</p>";
				echo "</div>";
