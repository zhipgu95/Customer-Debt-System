...
<link href="css/search_items_out_detail.css" rel="stylesheet"/>
</head>
<body>
	...
	<div id="menu">
		...
		//	$start = $total - $perpage;
			if(isset($_GET['search'])){
				$demander = $_GET['search'];
				$sql = "select Demander, Provider, YEAR(Date) as Year, MONTH(Date) as Month, Sum(Amount) as Amount
						from items_transfer 
						where Provider=8 and YEAR(Date)=2018 and Demander='$demander' group by MONTH(Date)";
			...
			if(!logged_in()){
				...
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["Demander"]."</td><td>".$row["Provider"]."</td><td>".$row["Year"]."</td><td>".$row["Month"]."</td><td>".$row["Amount"]."</td><td><a href='items_out_8_2018_newdetail.php?search=$row[Demander]&find=$row[Month]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>8号店2018年出货金额数(到".$demander."号店): ".$total_amount_one."</p>";
				echo "</div>";
