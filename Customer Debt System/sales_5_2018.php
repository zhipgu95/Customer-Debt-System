...
<div class = "include">
	...
	<div class = "table_info">
		...
		//	$start = $total - $perpage;
			...
			$sql = "SELECT Shop_no, YEAR(Date) AS year, MONTH(Date) AS month, SUM(Sales_amount) AS monthly_sales_amount, SUM(Costs_amount) AS monthly_costs_amount FROM sales_shop WHERE YEAR(Date)=2018 AND Shop_no=5 GROUP BY month";
			...
			if(!logged_in()){
				...
				//output data of each row
				$total_sales_amount_2018 = 0;
				$total_costs_amount_2018 = 0;
				while($row = mysqli_fetch_array($result)){
					...
					$total_sales_amount_2018+=$row["monthly_sales_amount"];
					$total_costs_amount_2018+=$row["monthly_costs_amount"];
				}
			//	$total_debt+=$totaldebt_perpage;
				echo "</table>";
				echo "</div>";
				echo "<div id='debt_form'>";
					echo "<p>2018年总销售额: ".$total_sales_amount_2018."</p>";
					echo "<p>2018年总费用额: ".$total_costs_amount_2018."</p>";
				echo "</div>";
			}
			else {
				echo "0个查询结果";
		...
		<div class = "table3">
    	<form method = "Post" action = "sales_5.php" enctype="multipart/form-data">
