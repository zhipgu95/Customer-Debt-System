		...
		//	$start = $total - $perpage;
			if(isset($_GET['search'])){
				...
				$sql = "select distinct concat(LastName, ' ', FirstName) as name, YEAR(Date) as year, MONTH(Date) as month, SUM(Amount) as amount  
						...
						where YEAR(Date)=2018 and concat(LastName, ' ', FirstName)='$name' group by month";
			...
			if(!logged_in()){
				...
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["name"]."</td><td>".$row["year"]."</td><td>".$row["month"]."</td><td>".$row["amount"]."</td><td><a href='employee_2018_newdetail.php?search=$row[name]&find=$row[month]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>2018年(员工:".$name.")借款金额数: ".$total_amount_one."</p>";
				echo "</div>";
