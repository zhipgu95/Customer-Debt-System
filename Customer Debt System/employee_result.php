<?php
	...
	if(isset($_GET['submit']))
	{
		...
		$insertquery1 = "INSERT INTO search_employee_record
						(FirstName, LastName)
						VALUES('$firstName','$lastName')";
			
		if (!employee_exists($firstName, $lastName, $conn)){
			$error = "该员工不存在，请去首页添加!";
		}
...
<div class = "include">
	...
	<div class = "table_info">
		...
		//	$start = $total - $perpage;
			...
			$sql = "select distinct concat(er.LastName, ' ', er.FirstName) as name, er.Date as date, er.Amount as amount, er.Detail as detail 
					from employee_record as er 
					where er.FirstName='$firstName' and er.LastName='$lastName'";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>姓名</th><th>日期</th><th>金额</th><th>详细备注</th></tr>";
				//output data of each row
				$totaldebt_perpage=0;
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>总借款金额: ".$totaldebt_perpage."</p>";
				echo "</div>";
        ...
        <input type = "submit" name = "submit_first" value = "添加新借款"/>
		</form>
		</div>
