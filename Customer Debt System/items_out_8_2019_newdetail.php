	...
	<div id="menu">
		...
		//	$start = $total - $perpage;
			if(isset($_GET['search'])){
				...
				$month = $_GET['find'];
				$sql = "select Demander, Provider, Date, Amount, Transfer_Detail 
						from items_transfer 
						where Provider=8 and YEAR(Date)=2019 and Demander='$demander' and MONTH(Date)='$month'";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>需求方</th><th>提供方</th><th>日期</th><th>金额数</th><th>具体调货信息</th></tr>";
				//output data of each row
				$total_amount_one = 0;
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["Demander"]."</td><td>".$row["Provider"]."</td><td>".$row["Date"]."</td><td>".$row["Amount"]."</td><td>".$row["Transfer_Detail"]."</td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>8号店2019年".$month."月份出货金额数(到".$demander."号店): ".$total_amount_one."</p>";
				echo "</div>";
