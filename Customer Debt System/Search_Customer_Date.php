...
<div class = "include">
	...
	<div class = "table_info">
		...
		//	$total = $perpage * $page;
		//	$start = $total - $perpage;
			$sql = "SELECT distinct concat(c.LastName, ' ', c.FirstName) AS name, c.Gender AS gender, c.City AS city, SUM(cr.Amount) AS amount,  MAX(cr.Date) as max_date
					FROM customer_record AS cr, customer AS c WHERE cr.LastName=c.LastName AND cr.FirstName=c.FirstName 
					GROUP BY name ORDER BY max_date ASC";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>姓名</th><th>性别</th><th>所在城市</th><th>欠款金额</th><th>最近欠(还)款日期</th><th>具体查询</th></tr>";
				//output data of each row
				$totaldebt_perpage=0;
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["name"]."</td><td>".$row["gender"]."</td><td>".$row["city"]."</td><td>".$row["amount"]."</td><td>".$row["max_date"]."</td><td><a href='search_new_result.php?search=$row[name]'>点击查看</a></td></tr>";	
