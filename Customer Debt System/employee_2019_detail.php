	...
	<div class = "table_info">
		...
		//	$start = $total - $perpage;
			if(isset($_GET['search'])){
				$name = $_GET['search'];
				$sql = "select distinct concat(LastName, ' ', FirstName) as name, YEAR(Date) as year, MONTH(Date) as month, SUM(Amount) as amount  
						from employee_record 
						where YEAR(Date)=2019 and concat(LastName, ' ', FirstName)='$name' group by month";
			...
			if(!logged_in()){
				...
				echo "<table><tr><th>姓名</th><th>年份</th><th>月份</th><th>金额数</th><th>具体信息</th></tr>";
				//output data of each row
				$total_amount_one = 0;
				while($row = mysqli_fetch_array($result)){
					echo "<tr><td>".$row["name"]."</td><td>".$row["year"]."</td><td>".$row["month"]."</td><td>".$row["amount"]."</td><td><a href='employee_2019_newdetail.php?search=$row[name]&find=$row[month]'>点击查看</a></td></tr>";	
			...
			//	$total_debt+=$totaldebt_perpage;
				...
				echo "<div id='debt_form'>";
					echo "<p>2019年(员工:".$name.")借款金额数: ".$total_amount_one."</p>";
				echo "</div>";
